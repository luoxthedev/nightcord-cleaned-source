...
// ── Updater stubs (no-op — updater removed) ──
ipcMain.on(IpcEvents.GET_INSTALLER_PREFS, (e) => {
    e.returnValue = { defaultPlugins: true };
});
ipcMain.handle(IpcEvents.GET_UPDATES, (event) => {
    if (!validateSender(event)) throw new Error("Unauthorized IPC invocation");
    return { ok: true, value: [] };
});
ipcMain.handle(IpcEvents.UPDATE, (event) => {
    if (!validateSender(event)) throw new Error("Unauthorized IPC invocation");
    return { ok: true, value: false };
});
ipcMain.handle(IpcEvents.BUILD, (event) => {
    if (!validateSender(event)) throw new Error("Unauthorized IPC invocation");
    return { ok: true, value: false };
});
ipcMain.handle(IpcEvents.GET_REPO, (event) => {
    if (!validateSender(event)) throw new Error("Unauthorized IPC invocation");
    return { ok: true, value: "" };
});
...
// Nightcord updater removed — stub
ipcMain.handle(IpcEvents.NIGHTCORD_DOWNLOAD_AND_RUN, async (event) => {
    if (!validateSender(event)) throw new Error("Unauthorized IPC invocation");
    return false;
});
