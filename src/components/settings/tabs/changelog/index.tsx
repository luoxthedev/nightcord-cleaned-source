...
function HashLink({ repo, hash, disabled }: { repo: string; hash: string; disabled: boolean; }) {
    const shortHash = hash.slice(0, 7);
    if (disabled || !repo) return <code>{shortHash}</code>;
    return <code>{shortHash}</code>;
}
...
function ChangelogContent() {
    ...
    React.useEffect(() => {
        if (repoErr) {
            console.error("Failed to retrieve repo", repoErr);
    ...
    const fetchChangelog = React.useCallback(async () => {
        ...
        try {
            ...
            console.error("Failed to fetch commits from repository", err);
            const errorMessage =
                err?.message ||
